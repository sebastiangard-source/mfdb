...
with sync_playwright() as p:
    ...
    pg.goto(url+'#boutiques=cape-and-islands'); pg.reload(); pg.wait_for_timeout(500)
    rl2 = pg.evaluate("[...document.querySelectorAll('#boBody a.bo-route')].map(a=>a.href)")
    ck('a region over ten shops splits into legs of at most ten', len(rl2)==2 and all(0 < h.count('/')-4 <= 10 for h in rl2), [h.count('/')-4 for h in rl2])
    ...
    pg.goto(url+'#boutiques=cape-and-islands'); pg.reload(); pg.wait_for_timeout(500)
    ck('a region hash opens the index filtered', pg.evaluate("document.querySelector('#boBody h2').textContent")=='Cape and Islands')
    pg.goto(url+'#boutiques'); pg.reload(); pg.wait_for_timeout(500)
