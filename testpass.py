...
with sync_playwright() as p:
    ...
    head('price states')
    pg.set_viewport_size({'width':1400,'height':1000})
    pg.goto(url+'#brand=Ring%20Jacket'); pg.reload(); pg.wait_for_timeout(400)
    rows = pg.evaluate("[...document.querySelectorAll('#detailBody .pr-row')].map(r=>r.innerText.replace(/\\n/g,' '))")
    ...
    pg.click('#boBody .bo-line a.shop-open'); pg.wait_for_timeout(400)
    ...
    pg.goto(url); pg.reload(); pg.wait_for_timeout(500)
    pg.mouse.move(5,5); pg.wait_for_timeout(400)   # a card open under a stale hover hides the search box by design
    _fo = pg.evaluate("closeAllPanels(); if(document.activeElement) document.activeElement.blur(); window.scrollTo(0,0); document.getElementById('brandSearch').focus(); [document.activeElement.id, innerWidth, innerHeight, document.hasFocus(), getComputedStyle(document.getElementById('brandSearch')).display, getComputedStyle(document.querySelector('.search-float')).display, getComputedStyle(document.querySelector('.search-float')).visibility, document.body.className, JSON.stringify(localStorage).slice(0,120)]"); pg.wait_for_timeout(200)
    pg.keyboard.type('herm'); pg.wait_for_timeout(300)
    _v = pg.evaluate("[document.getElementById('brandSearch').value, document.activeElement.id]")
    pg.keyboard.press('ArrowDown'); pg.keyboard.press('Enter'); pg.wait_for_timeout(600)
    ck('search box took focus and keystrokes', _v[0]=='herm', [_fo,_v])
    _c = pg.evaluate("[hoverCard.classList.contains('pinned'), hoverCard.dataset.brand, document.getElementById('brandSearch').value, document.getElementById('acList').classList.contains('open'), [...document.querySelectorAll('.brand.hit')].map(b=>b.textContent.trim()), document.querySelector('.brand.hit')?.classList.contains('out')]")
    ...
    ck('closing a brand page with the mouse returns focus to the chip', pg.evaluate("document.activeElement && document.activeElement.textContent.trim()==='Barbour'"))
    pg.goto(url); pg.reload(); pg.wait_for_timeout(500)
    pg.click('#boutiquesBtn'); pg.wait_for_timeout(400)
    pg.go_back(); pg.wait_for_timeout(500)
    _bk = [pg.evaluate("boutiquesView.classList.contains('show')"), pg.evaluate("location.hash"), pg.url[-40:]]
    ck('Back closes the stores index and stays on the page', (not _bk[0]) and _bk[1]=='' and pg.url.split('#')[0]==url.split('#')[0], _bk)
    ck('faded true-luxury brands are readable', pg.evaluate("parseFloat(getComputedStyle(document.querySelector('.band--true .brand') ).opacity)")>=0.9 and pg.evaluate("(()=>{const s=document.createElement('style'); s.textContent=''; const b=document.querySelector('.band--true .brand'); b.classList.add('out'); const o=parseFloat(getComputedStyle(b).opacity); b.classList.remove('out'); return o})()")>=0.29)
    ck('the search box is a landmark', pg.evaluate("document.querySelector('.search-float').getAttribute('role')")=='search')
    pg.set_viewport_size({'width':844,'height':390}); pg.goto(url); pg.reload(); pg.wait_for_timeout(500)
    ck('a phone held sideways gets the phone layout', pg.evaluate("getComputedStyle(document.querySelector('.layout')).display")!='grid')
    pg.set_viewport_size({'width':390,'height':844}); pg.goto(url+'#brand=Ralph%20Lauren%20Purple%20Label'); pg.reload(); pg.wait_for_timeout(500)
    ck('phone price axis shows every other label', pg.evaluate("[...document.querySelectorAll('#detailBody .pr-axis span')].filter(s=>getComputedStyle(s).display!=='none').length")<=6)
    pg.goto(url+'#stores'); pg.reload(); pg.wait_for_timeout(500)
    ck('phone region rows stack name over count', pg.evaluate("(()=>{const h=document.querySelector('#boBody h4.bo-reg'); const a=h.querySelector('.bo-toggle').getBoundingClientRect(), b=h.querySelector('.bo-count').getBoundingClientRect(); return b.top>=a.bottom-1})()"))
    ck('masthead buttons are tall enough to tap', pg.evaluate("[...document.querySelectorAll('.mastbtn')].every(b=>b.getBoundingClientRect().height>=36)"))
    pg.goto(url+'#prices'); pg.reload(); pg.wait_for_timeout(600)
    ...
    ck('the filtered index sets a shareable hash', pg.evaluate('location.hash') in ('#stores=south-shore+shop','#stores=south-shore+shop+az'))
    ...
    ck('typing keeps focus in the box', pg.evaluate("document.activeElement && document.activeElement.id==='boSearch'"))
    pg.fill('#boSearch',''); pg.wait_for_timeout(300)
    ...
    pg.click("#boBody button.bo-view[data-view='az']"); pg.wait_for_timeout(300)   # the flat-list checks below are about the A-to-Z view
